...
class BgeM3Embeddings(BaseModel, Embeddings):
    ...
    def embed_query(self, text: str) -> list[float]:
        text = text.replace("\n", " ")
        embedding = self.client.encode(text, **self.encode_kwargs)['dense_vecs']
...
class BgeReranker(BaseModel):
    ...
    def __init__(self, **kwargs: Any):
        ...
        if self.local_load:
            try:
                self.client: FlagReranker = FlagReranker(self.local_path, **self.model_kwargs)
            except EnvironmentError:
                logger.error('Load model from local fail. Download from huggingface...')

                self.client: FlagReranker = FlagReranker(self.model_name, cache_dir=self.local_path, **self.model_kwargs)
        else:
            self.client: FlagReranker = FlagReranker(self.model_name, **self.model_kwargs)
    ...
    def compress_documents(
        ...
        for i in range(0, len(sentence_pairs), 10):
            ...
            batch_scores = self.client.compute_score(batch_pairs, **self.encode_kwargs)
